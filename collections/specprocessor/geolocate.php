...
Language::load('collections/specprocessor/geolocate');
...
<html lang="<?= $LANG_TAG ?>">
	...
	<body>
		...
		<div role="main" id="innertext" style="background-color:white;">
			<h1 class="page-heading screen-reader-only"><?= $LANG['GEOLOCATE_COGE_EXPORT_MANAGER'] ?></h1>
			<?php
			if($collid && $isEditor){
				if($ACTIVATE_GEOLOCATE_TOOLKIT){
					...
					<form name="expgeolocateform" action="../download/downloadhandler.php" method="post" onsubmit="">
						<fieldset>
							<legend><b><?= $LANG['GEO_COM_TOOL'] ?></b></legend>
							<div style="margin:15px;">
								<?= $LANG['GEO_COM_TOOL_EXPLAIN'] ?>
							</div>
							<table>
								<tr>
									<td>
										<div style="margin:10px;">
											<b><?= $LANG['PROCESSING_STATUS'] ?>:</b>
										</div>
									</td>
									<td>
										<div style="margin:10px 0px;">
											<select name="processingstatus" onchange="cogeUpdateCount(this)">
												<option value=""><?= $LANG['ALL_RECORDS'] ?></option>
												<?php
 								...
 								<tr>
									...
									<td>
										...
										foreach($customArr as $i => $unitArr){
											...
											?>
											<div style="margin:10px 0px;">
												<select name="customfield<?= $i ?>" style="width:200px">
													<option value=""><?= $LANG['SELECT_FIELD'] ?></option>
													<option value="">---------------------------------</option>
													<?php
												...
												<select name="customtype<?= $i ?>" onchange="cogeUpdateCount(this)">
													<?php
												...
												<input name="customvalue<?= $i ?>" type="text" value="<?= $unitArr['v'] ?>" style="width:200px;" onchange="cogeUpdateCount(this)" />
											</div>
											<?php
										}
										?>
								...
								<tr>
									<td colspan="2">
										<fieldset style="margin:10px;padding:20px;">
											<legend><b><?= $LANG['COGE_STATUS'] ?></b></legend>
											<div>
												<b><?= $LANG['MATCH_COUNT'] ?>:</b>
												<?php
												...
												?>
												<span id="recalspan" style="color:orange;display:none;"><?= $LANG['RECALCULATING'] ?>... <img src="../../images/workingcircle.gif" style="width:13px;" /></span>
												<span style="margin-left:15px;"><button type="button" onclick="cogeUpdateCount(this)">Reset Count</button></span>
											</div>
											<div>
												<b><?= $LANG['COGE_AUTH'] ?>:</b>
												<span id="coge-status" style="width:150px;color:red;"><?= $LANG['DISCONNECTED'] ?></span>
												<span style="margin-left:40px"><button name="cogeCheckStatusButton" type="button" value="Check Status" onclick="cogeCheckAuthentication()"><?= $LANG['CHECK_STATUS'] ?></button></span>
												<span style="margin-left:40px"><a href="http://coge.geo-locate.org" target="_blank" onclick="startAuthMonitoring()"><?= $LANG['LOGIN_COGE'] ?></a></span>
											</div>
										</fieldset>
										<fieldset id="coge-communities" style="margin:10px;padding:10px;">
											<legend style="font-weight:bold"><?= $LANG['AVAILABLE_COMMS'] ?></legend>
											<div style="margin:10px;">
												<?= $LANG['TO_IMPORT_DATA'] ?>
											</div>
											<div style="margin:10px;">
												<div id="coge-commlist" style="margin:15px 0px;padding:15px;border:1px solid orange;">
													<span style="color:orange;"><?= $LANG['LOGIN_AND_CHECK'] ?></span>
												</div>
												<div id="coge-fieldDiv" style="display:none">
													<div style="margin:5px;clear:both;">
														<div style="float:left;"><?= $LANG['DATA_SOURCE_ID'] ?>:</div>
													...
													<div style="margin:5px;clear:both;">
														<div style="float:left;"><?= $LANG['DESCRIPTION'] ?>:</div>
										...
										<div style="margin:20px;clear:both;">
											...
											<div style="margin:5px">
												<button id="builddwcabutton" name="builddwcabutton" type="button" onclick="cogePublishDwca(this.form)" ><?= $LANG['PUSH_DATA'] ?></button>
												<span id="coge-download" style="display:none;color:orange"><?= $LANG['CREATING_PACKAGE'] ?>... <img src="../../images/workingcircle.gif" style="width:13px;" /></span>
												<span id="coge-push2coge" style="display:none;color:orange"><?= $LANG['PUSHING_TO_COGE'] ?>... <img src="../../images/workingcircle.gif" style="width:13px;" /></span>
												<span id="coge-importcomplete" style="display:none;color:green">
													<?= $LANG['SUCCESS_GEOLOCATE'] ?>
												</span>
											</div>
											<div style="margin-left:15px">
												...
												<div id="coge-importstatus" style="color:orange;display:none;">
													<?= $LANG['DATA_IMPORT_COMPLETE'] ?>
												</div>
											</div>
											<div style="margin:5px">
												<button name="submitaction" type="submit" value="Download Records Locally" ><?= $LANG['DOWNLOAD_LOCALLY'] ?></button>
											</div>
											<div style="margin:5px">
												<button name="resetbutton" type="button" value="Reset Page" onclick="cogeCheckAuthentication(); return false;" ><?= $LANG['RESET_PAGE'] ?></button>
											</div>
										</div>
										<div style="margin:20px;">
											<a href="../editor/editreviewer.php?collid=<?= $collid ?>&display=2"><?= $LANG['REVIEW_APPROVE_EDITS'] ?></a>
										</div>
										<div style="margin:20px;">
											<b>* <?= $LANG['DEFAULT_QUERY'] ?></b>
										</div>
									</td>
								</tr>
							</table>
