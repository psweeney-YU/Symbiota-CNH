<?php
/*
------------------
Language: Français (French)
Translated by: Google Translate (2024-09-16)
------------------
*/

$LANG['SITEMAP'] = 'Plan du Site';
$LANG['HOME'] = 'Accueil';
$LANG['COLLECTIONS'] = 'Collections';
$LANG['SEARCHENGINE'] = 'Moteur de Recherche';
$LANG['SEARCH_COLL'] = 'Rechercher des Collections';
$LANG['LISTOFCOLL'] = 'Liste des Collections Participant au Projet';
$LANG['COLLSTATS'] = 'Afficher les Statistiques Publiques des Collections';
$LANG['EXSICC'] = 'Indice Exsiccatae';
$LANG['DATA_PUBLISHING'] = 'Publication de Données';
$LANG['COLLECTIONS_RSS'] = 'Flux RSS pour les collections d\'histoire naturelle et les projets d\'observation';
$LANG['DARWINCORE'] = 'Archives Darwin Core (DwC-A)';
$LANG['RSS'] = 'Flux RSS DwC-A';
$LANG['PUBDATA'] = 'ensembles de données publiés de collections sélectionnées';
$LANG['IMGLIB'] = 'Bibliothèque multimédia';
$LANG['IMAGE_SEARCH'] = 'Outil de Recherche Interactif';
$LANG['CONTRIB'] = 'Contributeurs multimédia';
$LANG['USAGEPOLICY'] = 'Politique d\'Utilisation et Informations sur Droits d\'Auteur';
$LANG['ADDITIONAL_RESOURCES'] = 'Ressources Supplémentaires';
$LANG['GLOSSARY'] = 'Glossaire';
$LANG['PROTECTED_SPECIES'] = 'Espèces Protégées';
$LANG['LISTOFTAXA'] = 'liste des taxons où la localité et/ou les informations taxonomiques sont protégées en raison de leur statut rare/menacé/en danger';
$LANG['BIOTIC_INVENTORIES'] = 'Projets d\'Inventaire Biotique';
$LANG['ALL_CHECKLISTS'] = 'Toutes Listes Publiques';
$LANG['DYNAMIC'] = 'Listes d\'Espèces Dynamiques';
$LANG['CHECKLIST'] = 'Liste Dynamique';
$LANG['BUILDCHECK'] = 'construire dynamiquement une liste à l\'aide d\'enregistrements de spécimens géoréférencés';
$LANG['DYNAMICKEY'] = 'Clé Dynamique';
$LANG['BUILDDKEY'] = 'créer dynamiquement une clé à l\'aide d\'enregistrements de spécimens géoréférencés';
$LANG['MANAGTOOL'] = 'Outils de Gestion des Données';
$LANG['ADMIN'] = 'Fonctions Administratives (Super-Administrateurs Uniquement)';
$LANG['USERPERM'] = 'Autorisations Utilisateur';
$LANG['TAXONOMIC_CLEANER'] = 'Nettoyeur de Noms Taxonomiques à Collection Multiple';
$LANG['CREATENEWCOLL'] = 'Créer Nouvelle Collection ou Nouveau Profil d\'Observation';
$LANG['GEOTHESAURUS'] = 'Geographic Thesaurus';
$LANG['COORDVALIDATOR'] = 'Vérifier coordonnées par rapport aux frontières politiques';
$LANG['GUIDMAP'] = 'Mappeur de GUID';
$LANG['THUMBNAIL_BUILDER'] = 'Outil de Création de Vignettes';
$LANG['SALIX'] = 'Gestionnaire WordStat SALIX';
$LANG['IDKEYS'] = 'Clés d\'Identification';
$LANG['KEYMODULE'] = 'Remarque: le module Clé d\'Identification est désactivé dans ce portail. Cependant, vous pouvez annuler en activant des clés individuelles dans la page d\'administration de la liste.';
$LANG['AUTHOKEY'] = 'Vous êtes autorisé à accéder au';
$LANG['CHARASTATES'] = 'Éditeur de Caractères et d\'États de Caractère';
$LANG['AUTHIDKEY'] = 'Vous êtes autorisé à modifier les Clés d\'Identification';
$LANG['CODINGCHARA'] = 'Pour coder des caractères dans un format de tableau, ouvrez l\'éditeur de matrice pour l\'une des listes suivantes';
$LANG['NOTAUTHIDKEY'] = 'Vous n\'êtes pas autorisé à modifier les Clés d\'Identification';
$LANG['IMAGES'] = 'Multimédia';
$LANG['SEESYMBDOC'] = 'Voir la documentation Symbiota sur';
$LANG['IMGSUB'] = 'Soumission multimédia';
$LANG['FORANOVERVIEW'] = 'pour un aperçu de la façon dont le multimédia est géré dans un portail de données Symbiota. Le multimédia de terrain sans informations détaillées sur la localité peut être téléchargé à l\'aide de la page Profil d\'espèce de taxon.                                                Les spécimens multimédias sont chargés via la page d\'édition des spécimens ou via un processus de téléchargement par lots établi par un gestionnaire de portail. Les observations multimédias (bons multimédias) avec des informations détaillées sur la localité peuvent être téléchargées en utilisant le lien ci-dessous. Notez que vous aurez besoin des autorisations nécessaires pour utiliser cette fonctionnalité. ';
$LANG['BASICFIELD'] = 'Soumission multimédia de terrain de base';
$LANG['IMGOBSER'] = 'Module de soumission d\'observations multimédia';
$LANG['ADDNEWPROJ'] = 'Ajouter Nouveau Projet';
$LANG['LISTOFCURR'] = 'Liste des Projets en Cours';
$LANG['CLICKEDIT'] = '(cliquez pour modifier)';
$LANG['NOPROJ'] = 'Il n\'y a aucun projet dans le système';
$LANG['NOTEDITPROJ'] = 'Vous n\'êtes pas autorisé à modifier les Projets';
$LANG['TAXONPROF'] = 'Page de Profil du Taxon';
$LANG['THEFOLLOWINGSPEC'] = 'Les fonctions d\'édition de la Page de Profil d\'Espèce suivantes						sont également disponibles pour les éditeurs via un lien d\'édition situé						dans le coin supérieur droit de chaque Page de Profil du Taxon.';
$LANG['NOTAUTHOTAXONPAGE'] = 'Vous n\'êtes pas encore autorisé à modifier le profil du taxon';
$LANG['TAXONOMY'] = 'Taxonomie';
$LANG['TAXTREE'] = 'Visualiseur d\'Arbre Taxonomique';
$LANG['DYNTAXTREE'] = 'Explorateur de Taxonomie';
$LANG['EDITTAXPL'] = 'Modifier Placement Taxonomique (utiliser';
$LANG['TAXTREEVIEW'] = 'Visualiseur d\'Arbre Taxonomique)';
$LANG['ADDTAXANAME'] = 'Ajouter Nouveau Nom Taxonomique';
$LANG['BATCHTAXA'] = 'Téléchargement par lots d\'un Fichier de Données Taxonomiques';
$LANG['EOLLINK'] = 'Gérer Liens avec Encyclopédia of Life';
$LANG['NOTEDITTAXA'] = 'Vous n\'êtes pas autorisé à modifier la taxonomie';
$LANG['CHECKLISTS'] = 'Listes';
$LANG['TOOLSFORMANAGE'] = 'Des outils de gestion des listes sont disponibles à partir de chaque page d\'affichage des listes. Symboles d\'édition situés dans le coin supérieur droit de la
	page afficheront les options d\'édition pour cette liste. Vous trouverez ci-dessous une liste des listes que vous êtes autorisé à modifier.';
$LANG['EXSICCATII'] = 'Exsiccati';
$LANG['ESCMOD'] = 'Le Module Exsiccati est activé pour ce portail. L\'index exsiccati (listé ci-dessous) peut être consulté ou recherché par tout le monde. Cependant, pour
	ajouter ou modifier des titres ou des séries exsiccati, l\'utilisateur doit être administrateur pour au moins une collection.';
$LANG['NOTEDITCHECK'] = 'Vous n\'êtes autorisé à modifier aucune des listes';
$LANG['COLLECTION_DESCRIPTION'] = "Les collections sont organisées par type (par ex. : spécimens conservés, spécimens fossiles, observations et observations de terrain issues de la recherche).
	Les observations de terrain issues de la recherche constituent un type de collection spécialisé qui permet aux chercheurs de terrain de gérer leurs données et de
	générer des étiquettes pour les spécimens ou les observations, le tout étant étroitement lié à leur compte utilisateur. Pour plus d'informations, consultez la";
$LANG['SYMBDOCU'] = 'Documentation Symbiote';
$LANG['PRESERVED_SPECIMENS'] = 'Spécimens conservés';
$LANG['FOSSIL_SPECIMENS'] = 'Spécimens fossiles';
$LANG['OBSERVATIONS'] = 'Observations';
$LANG['GENERAL_OBSERVATIONS'] = 'Observations de terrain';
$LANG['NOEDITCOLL'] = 'Vous n\'avez pas d\'autorisations d\'édition explicites pour une collection particulière';
$LANG['PLEASE'] = 'S\'il te plaît';
$LANG['LOGIN'] = 'vous connecter';
$LANG['TOACCESS'] = ' pour accéder aux outils d\'édition.';
$LANG['CONTACTPORTAL'] = 'Contactez un administrateur de portail pour obtenir des autorisations de modification';
$LANG['ABOUT'] = 'À Propos de Symbiota';
$LANG['CODE'] = 'Version du Dodes';
$LANG['SCHEMA'] = 'Version du Schéma';
$LANG['AUTHO'] = 'Vous êtes autorisé à accéder au';
$LANG['VERNAC_COM'] = 'Modifier Noms Vernaculaires/Communs';
$LANG['TEXTDESC'] = 'Modifier Descriptions Textuelles';
$LANG['EDITIMG'] = 'Modifier le multimédia';
$LANG['IMGSORTORD'] = 'Modifier l\'ordre de tri multimédia';
$LANG['ADDNEWIMG'] = 'Ajouter un nouveau multimédia';
$LANG['DATASETS'] = 'Conjuntos de datos';
$LANG['ALLPUBDAT'] = 'Tous les Ensembles de Données Consultables Publiquement';
$LANG['DATMANPAG'] = 'Page de Gestion des Ensembles de Données';
$LANG['DATA_AUTHORIZED_TO_EDIT'] = 'ensembles de données que vous êtes autorisé à modifier';
$LANG['MANAGE_TAXON_THUMBNAILS'] = 'Gérer les miniatures de la carte du profil de taxonomie';
$LANG['TAXINTER'] = 'Autorisations des Utilisateurs d\'Intérêt Taxonomique';
$LANG['OTHER_CAT_TRANSFER'] = 'Outil de transfert de numéro de catalogue autre';
$LANG['BATCH_UPDATE_STATS'] = 'Mettre à jour les statistiques de la collection à l\'échelle du portail';
$LANG['POLICIES'] = 'Politiques';

?>