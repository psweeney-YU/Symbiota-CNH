...
$(document).ready(function () {
  ...
  if (/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent)) {
    ...
    if (ffversion < 7)
      alert(
        "You are using an older version of Firefox. For best results, we recommend that you update your browser."
      );
  }
  ...
  if (cookies.indexOf("localauto") > -1) {
    ...
    for (var i = 0; i < ca.length; i++) {
      ...
      if (c.indexOf(cookieName) == 0) {
        if (c.substring(cookieName.length) == "1")
          $("input[name=localautodeactivated]").prop("checked", true);
      }
    }
  }
...
function parseVerbatimCoordinates(f, verbose) {
  if (f.verbatimcoordinates.value) {
    ...
    if (!latDec || !lngDec) {
      var llEx1 =
        /([NSEW]{0,1})(\d{1,2})[\D\s]{1,2}(\d{1,2}\.{0,1}\d*)['m]{1}\s*(\d{1,2}\.{0,1}\d*)['"]{1,2}\s*([NSEW]{0,1})[\D\s]*(\d{1,3})[\D\s]{1,2}(\d{1,2}\.{0,1}\d*)['m]{1}\s*(\d{1,2}\.{0,1}\d*)['"]{1,2}\s*([NSEW]{0,1})/i;
      var llEx2 =
        /([NSEW]{0,1})(\d{1,2})[\D\s]{1,2}(\d{1,2}\.{0,1}\d*)['m]{1}\s*([NSEW]{0,1})\D*\s*(\d{1,3})[\D\s]{1,2}(\d{1,2}\.{0,1}\d*)['m]{1}\s*([NSEW]{0,1})/i;
      var llEx3 =
        /([NSEW]{0,1})([-]{0,1}\d{1,2}\.\d+)[\s]{0,1}([NSEW]{0,1})[\s]+([-]{0,1}\d{1,3}\.\d+)[\s]{0,1}([NSEW]{0,1})/i;
  ...
  if (f.editedfields && f.editedfields.value == "") {
    setTimeout(function () {
      if (f.editedfields.value) {
        ...
        alert(
          "No fields appear to have been changed. If you have just changed the scientific name field, there may not have enough time to verify name. Try to submit again."
        );
      }
    }, 1000);
    return false;
...
function verifyMinimumElevationInMeters(f) {
  ...
  if (parseInt(f.minimumelevationinmeters.value) > 8000) {
    alert(
      "Was this collection really made above the elevation of Mount Everest?"
    );
    return false;
  }
  return true;
  ...
  $.ajax({
    ...
    if (vList != "") {
      ...
      for (var key in vList) {
        strOut =
          strOut +
          "<li><a href='../../checklists/checklist.php?clid=" +
          key +
          "' target='_blank'>" +
          vList[key] +
          "</a></li>";
...
function eventDateChanged(eventDateInput) {
  ...
  if (
    !eventDateInput.form.recordnumber.value &&
    eventDateInput.form.recordedby.value
  )
    autoDupeSearch();
}
...
function validateDate(eventDateInput) {
  ...
  if (dateStr != "") {
    ...
    if (dateArr["y"] == 0) {
      alert(
        "Unable to interpret Date. Please use the following formats: yyyy-mm-dd, mm/dd/yyyy, or dd mmm yyyy"
      );
      return false;
    } else {
      ...
      try {
        ...
        if (testDate > today) {
          alert(
            "Was this specimen really collected in the future? The date you entered has not happened yet. Please revise."
          );
          return false;
        }
      } catch (e) {}
...
function openOccurrenceSearch(target) {
  ...
  occWindow = open(
    "../misc/occurrencesearch.php?targetid=" + target + "&collid=" + collId,
    "occsearch",
    "resizable=1,scrollbars=1,toolbar=0,width=750,height=600,left=200,top=40"
  );
  occWindow.focus();
